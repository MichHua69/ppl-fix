                            ...
                            <div class="">
                                <div class="flex flex-col mb-4 w-full justify-between ">
                                    ...
                                    <div class="relative basis-9/12 mt-2" data-twe-input-wrapper-init>
                                        <input type="text" name="nama" id="nama" placeholder="Masukkan Nama Program"
                                            ...
                                            value="{{old('nama', $program->nama_program ?? '')}}" autocomplete="nama" />
                                        @error('nama')
                                ...
                                <div class="flex flex-col mb-4 w-full justify-between">
                                    ...
                                    <input id="bodys" type="hidden" name="deskripsi"
                                        class="@error('deskripsi') border-red-500 @enderror"
                                        value="{{old('deskripsi', $program->deskripsi ?? '')}}">
                                    <trix-editor input="bodys" class="min-h-96"
                                        placeholder="Masukkan Deskripsi Program"></trix-editor>
                                    @error('deskripsi')
                                    <p class="text-red-500 text-xs italic">{{ $message }}</p>
                                    @enderror
                                </div>
                                <div class="flex flex-col mb-4 w-full justify-between ">
                                    <div class="relative flex basis-9/12 mt-2 justify-between items-center"
                                        data-twe-input-wrapper-init>
                                        <label class="basis-3/12 font-semibold text-xl flex items-center" for="judul">
                                            Jadwal
                                        </label>
                                        <button
                                            class="inline-block rounded rounded-lg text-center py-2 text-xl font-bold uppercase leading-normal text-white shadow-dark-3 transition duration-150 ease-in-out bg-primary hover:bg-primary-light min-w-32"
                                            type="button" id="add-button" onclick=""
                                                    ...
                                                    <tr>
                                                        <td class="border border-gray-300 px-4 py-2">
                                                            <input type="hidden" name="jadwal_id[]"
                                                                ...
                                                                value="{{ old('id.'.$i, $jadwalprogram[$i]->id ?? '') }}">
                                                            <input type="text" name="sesi[]"
                                                                class="w-full border rounded-md px-2 py-1 @error('sesi.'.$i) border-red-500 @enderror"
                                                                placeholder="Masukkan Nama Sesi"
                                                                value="{{ old('sesi.'.$i, $jadwalprogram[$i]->sesi ?? '') }}">
                                                            @error('sesi.'.$i)
                                                            <p class="text-red-500 text-xs italic">{{ $message }}</p>
                                                            @enderror
                                                        </td>
                                                        <td class="border border-gray-300 px-4 py-2">
                                                            <input type="date" name="tanggal[]"
                                                                class="w-full border rounded-md px-2 py-1 @error('tanggal.'.$i) border-red-500 @enderror"
                                                                value="{{ old('tanggal.'.$i, $jadwalprogram[$i]->tanggal ?? '') }}">
                                                            @error('tanggal.'.$i)
                                                            <p class="text-red-500 text-xs italic">{{ $message }}</p>
                                                            @enderror
                                                        </td>
                                                        <td class="border border-gray-300 px-4 py-2">
                                                            <input type="time" name="waktu_mulai[]"
                                                                class="w-full border rounded-md px-2 py-1 @error('waktu_mulai.'.$i) border-red-500 @enderror"
                                                                value="{{ old('waktu_mulai.'.$i, isset($jadwalprogram[$i]) ? \Carbon\Carbon::parse($jadwalprogram[$i]->waktu_mulai)->format('H:i') : '') }}"
                                                        ...
                                                        <td class="border border-gray-300 px-4 py-2">
                                                            <input type="time" name="waktu_selesai[]"
                                                                class="w-full border rounded-md px-2 py-1 @error('waktu_selesai.'.$i) border-red-500 @enderror"
                                                                value="{{ old('waktu_selesai.'.$i, isset($jadwalprogram[$i]) ? \Carbon\Carbon::parse($jadwalprogram[$i]->waktu_selesai)->format('H:i') : '') }}"
                                                                pattern="(?:[01]\d|2[0123]):(?:[012345]\d)">
                                                        ...
                                                        <td class="border border-gray-300 px-4 py-2">
                                                            <select name="puskeswan[]"
                                                                class="w-full border rounded-md px-2 py-1 @error('puskeswan.'.$i) border-red-500 @enderror">
                                                                <option value="{{$jadwalprogram[$i]->puskeswan->id}}" selected hidden>{{$jadwalprogram[$i]->puskeswan->puskeswan}}</option>
                                                                @foreach($puskeswan as $data)
                                                                <option value="{{$data['id']}}"
                                                                    {{ old('puskeswan.'.$i, $jadwalprogram[$i]->puskeswan->puskeswan ?? '') == $data['id'] ? 'selected' : '' }}>
                                                                    {{$data['puskeswan']}}</option>
                                                                @endforeach
                                                            </select>
                                                            @error('puskeswan.'.$i)
                                                            <p class="text-red-500 text-xs italic">{{ $message }}</p>
