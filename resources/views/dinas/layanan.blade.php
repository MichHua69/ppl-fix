                      ...
                      <table class="w-full divide-y divide-gray-200 dark:divide-gray-700 ">
                          ...
                          <tbody class="bg-white divide-y divide-gray-200 dark:divide-gray-700 dark:bg-gray-900">
                            @foreach ($puskeswan as $item)
                                <tr data-id="{{ $item->id }}">
                                  <td class="text-center px-4 nama-column">{{ $item->puskeswan }}</td>
                                  <td class="text-center px-4 namapengguna-column">{{ $item->alamat->jalan}}, {{$item->alamat->dusun}}, {{$item->alamat->wilayah->desa->desa}}, Kec. {{$item->alamat->wilayah->kecamatan->kecamatan}}</td>
                                  <td class="text-center px-4 email-column">{{ $item->telepon}}</td>
                                  <td class="py-4 px-4 text-center flex justify-center items-center gap-4">
                                    <a class="flex justify-center edit-icon" onclick="openEditModal({{ $item->id }}, '{{ $item->puskeswan }}', '{{ $item->alamat->jalan }}', '{{ $item->alamat->wilayah->kecamatan->id }}', '{{ $item->alamat->wilayah->desa->id }}', '{{ $item->alamat->dusun }}', '{{ $item->telepon }}')" href="">
                                      ...
                                      <a class="remove-icon" onclick="openRemoveModal({{ $item->id }})" href="">
    ...
    <div class="bg-white p-8 rounded shadow-lg w-1/3">
        ...
        <form id="formTambah" action="{{route('dinas.editakundokter')}}" method="POST">
          @csrf
            ...
            <div class="mb-4">
                <label for="nama" class="block text-sm font-medium text-gray-700">Nama PUSKESWAN</label>
...
<script>
  ...
  function openEditModal(id_pengguna, nama, jalan, kecamatan, desa, dusun, telepon) {
      document.getElementById("modaleditdata").classList.remove("hidden");
      document.querySelector("#modaleditdata [name='id_pengguna']").value = id_pengguna;
      document.querySelector("#modaleditdata [name='nama']").value = nama;
  ...
  function openRemoveModal(id_pengguna) {
      document.getElementById("modalremove").classList.remove("hidden");
      document.querySelector("#modalremove [name='id_pengguna']").value = id_pengguna;
  }
