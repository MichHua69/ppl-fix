...
<body>
    ...
    <section class="h-full bg-primary">
        <form action="{{ route('dinas.profil.save') }}" method="post" enctype="multipart/form-data" class="container px-6 py-10 mx-auto">
            @csrf
            <div class="bg-primary pt-20 mt-6 h-full pb-1">
                <div class="container grid grid-cols-3 bg-white rounded-lg shadow-lg overflow-hidden h-[82vh] relative">
                    @if(session('success'))
                    <div class="bg-green-100 border border-green-400 text-green-700 px-4 py-3 rounded absolute top-0 left-0 right-0 alert-message" role="alert">
                        ...
                        <span id="close-button" class="absolute top-0 bottom-0 right-0 px-4 py-3">
                            <svg class="fill-current h-6 w-6 text-green-500" role="button" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 20 20">
                                <title>Close</title>
                                <path d="M14.348 14.849a1.2 1.2 0 0 1-1.697 0L10 11.819l-2.651 3.029a1.2 1.2 0 1 1-1.697-1.697l2.758-3.15-2.759-3.152a1.2 1.2 0 1 1 1.697-1.697L10 8.183l2.651-3.031a1.2 1.2 0 1 1 1.697 1.697l-2.758 3.152 2.758 3.15a1.2 1.2 0 0 1 0 1.697z" />
                            </svg>
                        </span>
                    </div>
                    @endif
                    <div class="flex flex-col items-center justify-center gap-10 py-4 p-6 border-primary">
                        ...
                        <button type="button" class="inline-block w-full rounded px-6 pb-2 pt-2.5 text-xl font-bold uppercase leading-normal text-white shadow-dark-3 transition duration-150 ease-in-out bg-primary hover:bg-primary-light" type="submit" id="edit-profile-button" data-twe-ripple-init data-twe-ripple-color="light">
                            Edit
                        </button>
                    </div>
                            ...
                            <div class="container flex flex-row text-xl">
                                <div class="flex flex-col px-7 py-5 basis-1/2">
                                    <div class="flex flex-col mb-8 w-full justify-between">
                                        <label class="basis-3/12 font-semibold" for="nama">Nama</label>
                                        ...
                                        <div class="relative basis-9/12" data-twe-input-wrapper-init>
                                            <input type="text" name="nama" id="nama" placeholder="Masukkan Nama" class="hidden peer block w-full p-3 pl-4 border border-gray-300 rounded-md shadow-sm focus:outline:none focus:ring-primary focus:border-primary @error('nama') border-red-500 @enderror" value="{{ $aktor->nama }}" autocomplete="name" autofocus />
                                            @error('nama')
                                            <p class="text-red-500 text-xs italic">{{ $message }}</p>
                                            @enderror
                                        </div>
                                    ...
                                    <div class="flex flex-col mb-8 w-full justify-between">
                                        ...
                                        <div class="relative basis-9/12" data-twe-input-wrapper-init>
                                            <input type="text" name="nama_pengguna" id="nama_pengguna" placeholder="Masukkan Nama Pengguna" class="hidden peer block w-full p-3 pl-4 border border-gray-300 rounded-md shadow-sm focus:outline:none focus:ring-primary focus:border-primary @error('nama_pengguna') border-red-500 @enderror" value="{{ $aktor->Pengguna->nama_pengguna }}" autocomplete="nama_pengguna" />
                                            @error('nama_pengguna')
                                            <p class="text-red-500 text-xs italic">{{ $message }}</p>
                                            @enderror
                                        </div>
                                    ...
                                    <div class="flex flex-col mb-8 w-full justify-between">
                                        ...
                                        <div class="relative basis-9/12" data-twe-input-wrapper-init>
                                            <input type="email" name="email" id="email" placeholder="Masukkan Email" class="hidden peer block w-full p-3 pl-4 border border-gray-300 rounded-md shadow-sm focus:outline:none focus:ring-primary focus:border-primary @error('email') border-red-500 @enderror" value="{{ $aktor->Pengguna->email }}" autocomplete="email" />
                                            @error('email')
                                            <p class="text-red-500 text-xs italic">{{ $message }}</p>
                                            @enderror
                                        </div>
                                    ...
                                    <div class="flex flex-col mb-8 w-full justify-between">
                                        ...
                                        <div class="relative basis-9/12" data-twe-input-wrapper-init>
                                            <input type="password" name="password" id="password" placeholder="Masukkan Kata Sandi" class="hidden peer block w-full p-3 pl-4 border border-gray-300 rounded-md shadow-sm focus:outline:none focus:ring-primary focus:border-primary @error('password') border-red-500 @enderror" autocomplete="new-password" />
                                            @error('password')
                                            <p class="text-red-500 text-xs italic">{{ $message }}</p>
                                            @enderror
                                        </div>
                                    ...
                                    <div class="flex gap-5">
                                        <button type="button" class="hidden inline-block w-full rounded rounded-full px-6 py-2 text-xl font-bold uppercase leading-normal text-white shadow-dark-3 transition duration-150 ease-in-out bg-danger hover:bg-primary-light" type="button" id="cancel-profile-button" data-twe-ripple-init data-twe-ripple-color="light">
                                            Batal
                                        </button>
                                        <button type="submit" class="hidden inline-block w-full rounded rounded-full px-6 py-2 text-xl font-bold uppercase leading-normal text-white shadow-dark-3 transition duration-150 ease-in-out bg-primary hover:bg-primary-light" type="submit" id="save-profile-button" data-twe-ripple-init data-twe-ripple-color="light">
                                            Simpan
                                        </button>
                                    </div>
                                </div>
