    ...
    <div class=" pt-20 mt-8 h-full">
      <div class="container px-6 py-10 mx-auto">
        <div class="container bg-white rounded-lg shadow-lg h-full min-h-[75vh] relative p-12 mb-16">
          <div class="block">
            <h2 class="text-5xl font-bold mb-4 grid grid-row-15 row-span-1">Edit Laporan</h2>
            <form action="{{route('dokter.storeeditlaporan',['id'=> $laporan->id])}}" method="post" enctype="multipart/form-data">
              @csrf
              <div class="">
                <div class="flex flex-col mb-4 w-full justify-between ">
                  ...
                  <div class="relative basis-9/12 mt-2"
                      ...
                      <input type="text" name="judul" id="judul"
                          ...
                          value="{{old('judul', $laporan->judul_laporan ?? '')}}" autocomplete="judul" />
                      @error('judul')
                ...
                <div class="flex flex-col mb-4 w-full justify-between ">
                  ...
                  <div class="relative basis-9/12 mt-2"
                      ...
                      <select 
                          ...
                          <option selected hidden value="{{old('peternak', $laporan->id_peternak ?? '')}}">Pilih peternak</option>
                          @foreach ($peternak as $item)
                          <option value="{{ $item->id_pengguna }}" {{ old('peternak', $laporan->id_peternak) == $item->id_pengguna ? 'selected' : '' }}>{{ $item->nama }}</option>
                          @endforeach
                      </select>
                ...
                <div class="flex flex-col mb-4 w-full justify-between">
                  ...
                  <input id="bodys" type="hidden" name="isi" class="@error('isi') border-red-500 @enderror" value="{{old('isi', $laporan->isi_laporan ?? '')}}">
                ...
                <div class="flex justify-end mt-8">
                  <div class="flex gap-5">
                    ...
                    type="button"
                    onclick="window.location='{{route('dokter.lihatlaporan', ['id' => $laporan->id])}}'"
